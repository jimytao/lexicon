...
### 用户可见
1. **输入过程中也能随时切换 IN / OUT**：方向胶囊不再因为输入框已有文字、已有搜索结果、辅助语言或无关语言而变灰；中文、越南语和英文都可以在提交前明确选择本次用途。
2. **提交后切换不反改上一条搜索**：按下搜索时会立刻冻结该条查询的方向快照；之后继续点击 IN / OUT 只会准备下一次搜索，不会改变已经发出的 AI Prompt、Profile 证据或异步返回结果的归属。
3. **AI Profile 不再把“AI 原样返回”当成用户犯错**：只有 OUT 英文原句与 `correctForm` 存在实质差异时才形成高权重订正证据；纯大小写、空白或装饰性标点变化会忽略，但 `dont → don't` 这类有语法意义的变化仍会保留。
4. **画像严格跟随当前有效词典语言**：英汉只使用中文学习者画像，英越只使用越南语学习者画像，英英只使用英语母语 / 单语画像；按单词、短语或句子开启英英覆盖时，也会立即切换到英英身份，不再把中文默认带入所有模式。
5. **AI 追问按原始查询判定身份**：中文或越南语查询即使被 AI 修正成英文，后续问答和 Profile 记录仍沿用原始查询的 OUT 路线与对应学习者语言，不会中途误切成英文 IN。
6. **Profile 状态表达更诚实**：热度分层改为“近期重复出现 / 仍需关注 / 近期未再出现”，时间衰减只影响提醒优先级，不再暗示“很久没出现就是掌握”；已掌握项不会继续注入实时 AI。
7. **手动归纳显示真实待处理数量**：设置中的操作会显示待归纳记录数；队列为空时明确显示“Profile 已是最新”，避免把一次无内容请求误解为画像刷新。
8. **IN / OUT 连续切换不再让历史记录闪烁**：方向按钮把光标交还搜索框后，会取消旧的延迟失焦任务；只要输入光标仍在空搜索框中，历史记录持续展开，真正离开输入框后才关闭。
9. **搜索框单行文字恢复垂直居中**：占位文字与真实输入在单行/折叠状态下和左侧方向按钮共用中心线；多行展开仍保持顶部对齐，不改变 textarea 宽度与右下角按钮绕行逻辑。
10. **AI 增加条件式文化表达识别**：在普通释义前先判断输入是否确有俚语、网络梗、文字游戏、谐音/故意错写或地域用法证据；命中时恢复源语言社群中的真实含义并给自然目标语表达，未命中时保持普通查词/翻译不变。

### 工程与验证
- `resolveLearningRoute()` 改为由手动方向拥有英文及当前词典辅助语言；与有效词典无关的第三语言仍为 `irrelevant`，不会进入 Profile。
- `handleWordSelect` 在任何 hydration / DB 异步等待之前捕获 `directionSnapshot`；组合单词、词组、重试、模式切换与强制 AI 路径都显式传递该次 `LearningRoute`，不再从可变 UI 状态重读。
- AI lookup / phrase API 的内部 options 支持显式 `learningRoute`；结果页 Profile 洞察、后续 AI Chat 与订正事件都沿用提交快照，不跟随下一次方向切换。
- 新增 `resolveLearnerLanguagePolicy()`，把有效词典、母语 / 辅助语言、Profile 语言与单语覆盖收敛为单一策略；越南语无重音输入采用保守证据判定，`không` 等高置信单词仍可识别，其他重音拉丁语不再误入越南语画像。
- Profile 事件、弱点、探索焦点与推荐新增 `learnerLanguage` 归属；诊断批次按方向和语言隔离，旧版无语言字段的数据只兼容归入中文分区，不会泄漏至英越或英英。
- 诊断开始时固定本轮事件快照；成功后只删除该快照，并按仍在队列中的 lookup 数重算计数。在途新增记录不会因旧请求返回而归零，剩余高价值事件会继续归纳。
- compact 注入最多 3 条热弱点与 2 条近期探索方向，full 注入最多 6 条 active 弱点与 3 条探索方向；两者都只能补充标准分析，不能删减或替换原有字段职责。
- 英越 Prompt 使用越南语解释与越南语迁移身份；英英 Prompt 只使用英语母语 / 单语身份。`unnaturalMindModel.chineseThought` 保留为旧 schema 字段名，但语义已统一为“源语言迁移思维”。
- 新增可取消延迟工具（`cancelableDelay`），SearchBar 聚焦时取消过期 blur 关闭，并在延迟回调执行前复核真实 DOM 焦点。
- 新增共享 `buildCultureAwareInputRule()`，注入 Stage-1 `resolveQuerySkeleton`、单词全量 Prompt 与词组/句子 Prompt；文化表达可返回自然英文短表达，不再被强制压成单个英文词。
- TDD 与全量回归：39 个测试文件、353 项测试全部通过；Web 与浏览器扩展生产构建通过；ESLint 0 error（67 个既有 warning）。

## 2026-09-24 — IN / OUT 学习方向与 Profile 证据隔离 (v0.9.26)

### 用户可见
1. **搜索栏新增单击式 IN / OUT 方向钮**：原放大镜位置改为紧凑方向胶囊；英文材料可随时在输入学习（IN）与主动表达（OUT）之间切换，点击后焦点立即回到输入框，不增加额外确认步骤。
2. **母语输入自动视为 OUT**：英汉 / 英英词典下的中文，以及英越词典下的越南语，自动锁定为 OUT；日语、韩语、德语等第三语言仍可查询或翻译，但不会写入英语学习画像。
3. **设置新增默认学习方向**：Local Data 区新增 `Default Learning Direction`，与 `Default Search Mode` 完全独立；设置默认值后，搜索栏仍可逐次覆盖。
4. **同一本 Profile 分成两条证据线**：IN 只记录理解、词汇与输入材料暴露出的缺口；OUT 只记录用户自己的表达、订正与主动用词。Profile 弹窗即使为空也明确展示两个分区。
5. **旧画像安全隔离**：没有方向标签的历史弱点继续保留可查看，但不再注入 AI Prompt；结果页洞察仅在方向与当前原始查询一致时显示，避免把教材原句误判为用户写作风格。
6. **修复浏览器扩展反向选词按钮位置**：同行从右往左、跨行从右下往左上选择文字时，浮动查词按钮现在跟随鼠标松开的 focus 端显示在选区左侧/左上方；正向选择仍停靠右侧/右下方。

### 工程与验证
- 新增 `LearningDirection` / `LearningRoute`、`resolveLearningRoute()` 纯函数与语言路由测试；`searchStore` 保存会话方向，`settingsStore` 持久化默认方向。
- Profile 事件、弱点、探索焦点与推荐均可携带 `learningDirection`；非学习语言事件直接丢弃，IN 句子与母语 OUT 查询不得生成语法错误证据。
- 所有 live AI 出口按当前方向过滤 Profile 上下文；AI 返回的 `profileInsightDirection` 随缓存保存，旧缓存洞察默认隐藏。
- 扩展选区方向改为比较 `Selection.anchor/focus` 的 DOM 顺序，不再仅凭 Y 坐标推断；新增 `selectionGeometry.ts` 与 11 项单元测试，覆盖同行/跨节点方向、focus 端 rect、双向定位、视口翻边及异常回退。
- TDD：37 个测试文件、322 项测试全部通过；TypeScript 与浏览器扩展生产构建通过；本次扩展选区文件 ESLint 0 error；本地 Chrome 使用真实 `content.js` 验证同行/跨行正反四种拖选方向，console 0 warning / 0 error。
...
5. **三种查词模式各自记住阅读位置**：同一次查询中，从 Instant、AI Lookup 或 Pure Core 切到另一模式后，再切回来会回到各自上次阅读的位置；第一次进入的模式从顶部开始。搜索新词、选择历史词条或强制 AI 时三种位置都会重置。
...
- 新增 `modeScrollMemory.ts` 与单元测试；`App.tsx` 在分段模式切换前保存当前位置，并在目标视图提交后的 `useLayoutEffect` 中恢复，新查询入口统一清空记录。
- 验证：34 个测试文件、297 项测试全部通过；主应用与浏览器扩展生产构建通过；产品源码 ESLint 0 error（保留 68 个既有 warning）。
...
- **新增 `eslint.config.mjs`（ESLint 9 flat config）**：安装 `eslint@^9`、`@eslint/js@^9`、`typescript-eslint@^8`、`eslint-plugin-react-hooks@^5`；配置 TypeScript + React Hooks 规则，忽略 `dist/`、`dist-ext/`、`dist-dictionaries/`、`node_modules/`、`public/`、`src-tauri/`、`android/`、`ios/`、`scripts/` 及所有 `*.config.*`；`any`、`!.` 非空断言等历史代码宽松设为 `warn`，未使用变量下划线前缀豁免。
- **修复 4 个 eslint error**：
  - `src/components/SearchBar/index.tsx`：三元表达式改 `if/else`（`no-unused-expressions`）
  - `src/services/audio.ts`：空 catch 块加 `_e` + 注释（`no-empty` × 2）
  - `src/utils/aiConversations.ts`：`let` → `const`（`prefer-const`）
