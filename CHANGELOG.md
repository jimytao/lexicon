...
## 2026-05-10 — Module Management 移动端拖曳排序优化

### 概述

本次更新修复了 Module Management 在移动端的可用性问题。此前排序按钮仅在鼠标 hover 时显示，导致 Android/iOS 用户无法排序。现在引入了拖曳排序功能，并为不同设备优化了交互方式。

---

### 1. `SettingsDrawer.tsx` — 移动端拖曳排序重构

- **引入 dnd-kit 拖曳库**：安装 `@dnd-kit/core`、`@dnd-kit/sortable`、`@dnd-kit/utilities`，实现跨平台拖曳排序。
- **三横杠拖曳手柄（Grip Handle）**：在模块列表项最右端常态显示三横杠图标（`|||`），这是移动端用户熟悉的拖曳指示器。仅该手柄可触发拖曳排序。
- **箭头按钮左移并保留 hover 逻辑**：上下箭头按钮移至三横杠左侧，仍保持 `opacity-0 group-hover:opacity-100`。桌面端 hover 显示箭头可微调顺序；移动端无 hover，箭头不可见也不可点击，避免误触。
- **多传感器支持**：配置 PointerSensor（鼠标）、TouchSensor（触摸）、KeyboardSensor（键盘），确保桌面和移动端都能流畅拖曳。
- **拖曳视觉反馈**：拖曳时模块项呈现半透明、阴影和边框高亮效果，提供清晰的交互反馈。
