...
## 0. 首次发版前置检查（仅首次需要）
**公钥验证**：确保 `src-tauri/tauri.conf.json` 中的 `plugins.updater.pubkey` 与当前使用的私钥 `src-tauri/lexicon.key` 匹配。公钥从私钥派生，只要私钥不变，公钥就恒定。
- 验证方法：运行 `npx tauri signer generate --password "<私钥密码>"` 生成测试密钥对，对比输出的 Public 与 `tauri.conf.json` 中的 pubkey 是否一致。
- **注意**：pubkey 不随编译变化，仅当更换私钥时才需要更新。不匹配会导致下载时签名验证失败。
