...
**最近一次重要改动**：2026-07-20，feat — 单词与短语发音（音频播放）支持与配置面板集成 (v0.7.33)：新增单词和短语的英音 (UK) 与美音 (US) 在线发音功能（对接有道 API），针对日、韩、中等提供通用标准发音；支持离线或加载失败时降级至设备原生 Web Speech TTS 播放，并设计了全局播放防重叠逻辑；在查词标题音标旁加入了 UK/US 和 Play 动态微动效发音按钮，点击或自动发音时会触发呼吸闪烁和小喇叭弹跳动画；新增「发音口音默认偏好」和「自动播放开关」的设置面板选项。
