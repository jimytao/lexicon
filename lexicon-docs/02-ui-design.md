...
- **长文本与 URL 防溢出**：所有标题（`h1`）、对比文本（`DiffText`）与容器强制使用 `break-words [overflow-wrap:anywhere]` 约束，防止无空格 URL 撑开 PC 窗口横向溢出。
- **长文本 UI 折叠**：针对 `correctForm` / `DiffText` 区域与 `meaning` 翻译文本，当文本数/行数较多时，默认显示前 4 行并带渐变遮罩，提供“展开全文/收起”控件。
