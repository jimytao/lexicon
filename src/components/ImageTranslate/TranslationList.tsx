...
export function TranslationList({ blocks, onUpdateTranslation }: Props) {
  const t = useT()
  if (blocks.length === 0) {
    return <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-4">{t('image.noText')}</p>
  }
    ...
    <div className="space-y-3">
      {blocks.map((block, i) => {
        ...
        return (
          <div
            key={i}
            className="rounded-lg border p-3 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30"
          >
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                ...
                {block.type === 'sfx' ? t('image.sfx') : block.type === 'caption' ? t('image.caption') : t('image.bubble')}
              </span>
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                {block.direction === 'vertical' ? t('image.vertical') : t('image.horizontal')}
              </span>
              {block.rotation ? (
