...
import { generateExercises, generateMeaningExercises, evaluateAnswer, evaluateMeaningCheck } from '../../../services/ai'
import type { Exercise, MeaningExercise } from '../../../types'
...
function MeaningCheckPractice({
  ...
  const [exercises, setExercises] = useState<MeaningExercise[]>([])
  ...
  async function handleGenerate() {
    ...
    setErrorMsg('')
    try {
      const result = await generateMeaningExercises(word, meanings, maxExercises, abortRef.current.signal)
      setExercises(result)
  ...
  async function handleSubmit(index: number) {
    ...
    try {
      const result = await evaluateMeaningCheck(
        word,
        meanings,
        answer,
        exercises[index].sentence,
        exercises[index].targetMeaning,
        abortRef.current?.signal
      )
      updateExState(index, {
        status: result.correct ? 'correct' : 'incorrect',
        feedback: result.feedback,
        correction: exercises[index].targetMeaning || '',
      })
    } catch (e) {
      if ((e as Error).name === 'AbortError') return
      ...
      {(sectionStatus === 'idle' || sectionStatus === 'error') && (
        <div>
          <button
            ...
            className="w-full text-sm py-2.5 rounded-xl border border-dashed border-border text-foreground-muted hover:bg-foreground/5 transition-colors cursor-pointer font-medium"
          >
            {t('practice.generate')}
          </button>
          {sectionStatus === 'error' && (
              ...
              <div key={i} className="rounded-xl border border-border/50 p-3">
                <div className="text-xs text-foreground-muted mb-2">
                  <div className="font-semibold text-foreground mb-1">
                    {t('practice.scenario')} {i + 1}:
                  </div>
                  <div className="font-medium text-foreground bg-accent/5 border border-accent/10 p-2 rounded-lg text-xs leading-relaxed">
                    “{ex.sentence}”
                  </div>
                  {ex.hint && (
                    <p className="text-[11px] text-foreground-muted/70 mt-1">💡 {ex.hint}</p>
                  )}
                </div>
                <textarea
                  rows={2}
                ...
                {st.status === 'correct' && (
                  <p className="mt-2 text-xs font-medium text-green-600 dark:text-green-400">
                    {t('practice.correct')} {st.feedback ? `— ${st.feedback}` : ''}
                  </p>
                )}
                  ...
                  <div className="mt-2 space-y-1.5">
                    ...
                    {st.correction && (
                      <p className="text-xs text-foreground-muted">
                        <span className="font-medium">{t('practice.reference')}</span>
                        {st.correction}
                      </p>
                    )}
                    <button
