          ...
          return (
            <div key={i} className="rounded-xl border border-gray-100 dark:border-gray-800 p-3">
              ...
              <textarea
                ...
                placeholder={t('phraseEx.placeholder')}
              ...
              {(st.status === 'idle' || st.status === 'evaluating') && (
                ...
                >
                  {st.status === 'evaluating' ? t('phraseEx.grading') : t('phraseEx.submit')}
                </button>
              )}
              {st.status === 'correct' && (
                <p className="mt-2 text-xs font-medium text-green-600 dark:text-green-400">{t('phraseEx.correct')}</p>
              )}
              {st.status === 'incorrect' && (
                <div className="mt-2 space-y-1.5">
                  <p className="text-xs text-red-500 dark:text-red-400">{st.feedback}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    <span className="font-medium">{t('phraseEx.reference')}</span>{st.correction}
                  </p>
                  <button
